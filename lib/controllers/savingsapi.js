...
var goals = db.collection('savings', {strict: true}, function(err,collection) {
		if (err) {
...
exports.createSavings = function(req,res) {
	...
	goals.insert(newSavings, function(err,result) {
	if (err) {
		console.log(err);
		throw err;
	}
...
exports.findAllByUser = function (req,res) {
	savings.find({userid: req.user._id}, function(err,result) {
		res.send(result);
	});
}
