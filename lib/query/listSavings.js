...
users.find({}).toArray(function(err, userlist) {
	...
	goals.find({}).toArray(function(err, goallist) {
		...
		savings.find({}).toArray(function(err, savinglist) {
			...
			var inactives=[];
			var activeUsers=[];
			var extremeActiveUsers=[];
			var actives= und.countBy(userlist, function(u) {
				if (u.savings&& !u.savings.length>1) {
			...
			var extremeActives= und.countBy(userlist, function(u) {
				if (u.savings && u.savings.length>=2) {
					...
					var uniquedays = und.countBy(u.savings, function(s) {
						return s.savingMoment.format("MMDD");
					})

					//console.log(u.username);
					//console.log("last Saving since create: "+latestSaving.savingMoment.subtract(u.createMoment).days());
					//console.log("last saving since now: "+moment().subtract(latestSaving.savingMoment).days())
					if (latestSaving.savingMoment.diff(u.createMoment, 'days')>=1 && moment().diff(latestSaving.savingMoment,'days')<=7 && und.size(uniquedays)>2) {

						extremeActiveUsers.push(u);
			...
			stats.extremeActives=extremeActives.extremeActive;
			...
			extremeActiveUsersjson=JSON.stringify(extremeActiveUsers,null,4);
			...
			fs.writeFile('extremeActives.json', extremeActiveUsersjson, function(err) {
					if (err) {
						return console.log(err);
					}
					console.log("Written to extremeActives.json");
				});
