		...
		savings.find({}).toArray(function(err, savinglist) {
			...
			var extremeActives= und.countBy(userlist, function(u) {
				if (u.savings.length>=3) {
					...
					console.log("last saving since now: "+moment().subtract(latestSaving.savingMoment).days()<3)
					if (latestSaving.savingMoment.subtract(u.createMoment).days()>2 && moment().subtract(latestSaving.savingMoment).days()<3) {
