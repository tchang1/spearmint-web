		...
		savings.find({}).toArray(function(err, savinglist) {
			...
			var extremeActives= und.countBy(userlist, function(u) {
				if (u.savings.length>=3) {
					...
					if (latestSaving.savingMoment.diff(u.createMoment, 'days')>=2 && moment().diff(latestSaving.savingMoment,'days')<=3 && und.size(uniquedays)>2) {
